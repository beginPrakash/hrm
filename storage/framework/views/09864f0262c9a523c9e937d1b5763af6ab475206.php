            ...
            <div class="card-body">
                ...
                <form action="<?php echo e(route('create_attendance_by_date')); ?>" method="post" id="create_attn_form">
                    ...
                    <div class="input-group timex timepickerx">
                        <?php $start_date = $emloyeeSchedule[0]['min_start_time'] ?? ''; ?>   
                        Start Time <?php if(!empty($start_date)): ?> (<?php echo e(date('d F, Y',strtotime($start_date))); ?>) <?php endif; ?>: <input type="text" class="form-control att_timepicker" name="start_time" id="start_time" autocomplete="off"><span class="input-group-text"><i class="fa fa-clock-o"></i></span><br>
                        <input type="hidden" name="schd_start_date" value="<?php if(!empty($start_date)): ?> <?php echo e(date('Y-m-d',strtotime($start_date))); ?> <?php endif; ?>">
                    </div>

                    <div class="input-group timex timepickerx">
                        <?php $end_date = $emloyeeSchedule[0]['min_end_time'] ?? ''; ?>
                        End Time <?php if(!empty($end_date)): ?> (<?php echo e(date('d F, Y',strtotime($end_date))); ?>) <?php endif; ?> : <input type="text" class="form-control att_timepicker" name="end_time" id="end_time" autocomplete="off"><span class="input-group-text"><i class="fa fa-clock-o"></i></span>
                        <input type="hidden" name="schd_end_date" value="<?php if(!empty($end_date)): ?> <?php echo e(date('Y-m-d',strtotime($end_date))); ?> <?php endif; ?>">
