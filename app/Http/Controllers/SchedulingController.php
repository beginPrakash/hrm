    ...
    public function store(Request $request)
    {
        ...
        foreach($empArray as $emp)
        {
            $insertArray = array(
                ...
                'min_start_time'    =>  ($request->shift_addschedule >2)?date('h:i:s a', strtotime(str_replace(' pm','',$request->min_start_time))):NULL,
                'start_time'        =>  ($request->shift_addschedule >2)?date('h:i:s a', strtotime(str_replace(' pm','',$request->start_time))):NULL,
                'max_start_time'    =>  ($request->shift_addschedule >2)?date('h:i:s a', strtotime(str_replace(' pm','',$request->max_start_time))):NULL,
                'min_end_time'      =>  ($request->shift_addschedule >2)?date('h:i:s a', strtotime(str_replace(' pm','',$request->min_end_time))):NULL,
                'end_time'          =>  ($request->shift_addschedule >2)?date('h:i:s a', strtotime(str_replace(' pm','',$request->end_time))):NULL,
                'max_end_time'      =>  ($request->shift_addschedule >2)?date('h:i:s a', strtotime(str_replace(' pm','',$request->max_end_time))):NULL,
            ...
            for($k=4; $k<$totalColumns; $k++)
            {
                ...
                if(!empty($shiftDetails))
                {
                    ...
                    $scheduleInsertArray = array(
                        ...
                        'shift'             =>  $shiftid,
                        'min_start_time'    =>  $shiftDetails->min_start_time,//date('h:i:s a', strtotime($importData[4])),
                        'start_time'        =>  $shiftDetails->start_time,//date('h:i:s a', strtotime($importData[5])),
                        'max_start_time'    =>  $shiftDetails->max_start_time,//date('h:i:s a', strtotime($importData[6])),
                        'min_end_time'      =>  $shiftDetails->min_end_time,//date('h:i:s a', strtotime($importData[7])),
                        'end_time'          =>  $shiftDetails->end_time,//date('h:i:s a', strtotime($importData[8])),
                        'max_end_time'      =>  $shiftDetails->max_end_time,//date('h:i:s a', strtotime($importData[9])),
