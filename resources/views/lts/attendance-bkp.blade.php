                ...
                <div class="content container-fluid">
                    ...
                    <div class="row">
                        <div class="col-lg-12 small-box-line">
                            ...
                            <div class="small-box red"></div><span class="smmr-6">Missing Punch || Absent </span>
                                        ...
                                        if(isset($attEmployees))
                                        {
                                            ...
                                            <tr>
                                                ...
                                                { 
                                                    ...
                                                    {
                                                        $abOff = getShiftName($shiftDetails->shift);
                                                        ?>
                                                            <td><?php echo $abOff; ?></td>
                                                        <?php
                                                        continue;
                                                    }
                                                    ...
                                                    { 
                                                        $shcolor = '';
                                                        if($emloyeeAttendance->day_type === 'work')
                                                        {
                                                            ...
                                                            if($firstclockin!=null && $lastclockout!=null)
                                                            {
                                                                $hour = 0;
                                                                $minutes = 0;
                                                                if($firstclockin->attendance_time!=='0' && $lastclockout->attendance_time!=='0')
                                                                {
                                                            ...
                                                            {   
                                                            ?>
                                                                <td><a href="javascript:void(0);" class="popupAttn" data-id="{{$encoded}}"><i class="fa fa-info-circle <?php echo $shcolor; ?>"></i></a></td>
                                                            <?php
                                                            }
                                                            else
                                                            {
                                                            ?>
                                                                <td><a href="javascript:void(0);" class="popupAttn" data-id="{{$encoded}}"><i class="fa fa-check text-success"></i></a></td>
                                                            <?php
                                                            }
                                                        }
                                                        else
                                                        {
                                                            if(!empty($shiftDetails))
                                                            {
                                                                $abOff = '<i class="fa fa-close text-danger"></i>';
                                                                if(in_array($shiftDetails->shift, array(1,2,3)))
                                                                {
                                                                    $abOff = getShiftName($shiftDetails->shift);
                                                                }
                                                            }
                                                            ...
                                                            {
                                                                $abOff = strtoupper($emloyeeAttendance->day_type);
                                                            }
                                                        ?>
                                                            <td><?php echo $abOff; ?></td>
                                                        <?php
                                                    ...
                                                    {
                                                        $leaveDate =$date;
                                                        $leaveData = App\Models\Leaves::select('e.dm_approval', 't.sort')
                                                                    ->from('employee_leaves as e')
                                                                    ->join('leave_type as t', 'e.leave_type', '=', 't.id')
                                                                    ->where('user_id',$employee->user_id)
                                                                    ->where('e.leave_from', '>=', $leaveDate)
                                                                    ->where('e.leave_to', '<=', $leaveDate)
                                                                    ->get()->first();
                                                                    
                                                                    // Approvel of DM $leaveData['dm_approval'] == 4
                                                        if(!empty($leaveData) &&  $leaveData['dm_approval'] == 4){ 
                                                            ?>
                                                            <td><a href="javascript:void(0);"><span class="first-off " >
                                                              <?=$leaveData['sort'];?>  
                                                            </span></a></td>
                                                     <?php   }else{
                                                    ?>
                                                        <td><a href="javascript:void(0);"><span class="first-off " style="color: red"> A</span></a></td>
                                                    <?php 
                                                    }
                                                    }
...
<script type="text/javascript">
    function saveOtApprove() {
       ...
       $.ajax({
           type: "POST",
           url: "{{ url('approveOt/') }}",
           data: {attnUserId:attnUserId, attnDate:attnDate, ottime:ottime, approve_status:approve_status, approve_remark:approve_remark, start_time:start_time, end_time:end_time, "_token": "{{ csrf_token() }}"},
