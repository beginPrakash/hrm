...
$(document).ready(function() {
	...
	$(document).on('click', '.leave-edit-btn', function() {
		...
		var input_type = $(this).data('data');
		$(this).removeClass('leave-edit-btn').addClass('btn btn-white leave-cancel-btn').text('Cancel');
		$(this).closest("div.leave-right").append('<button class="btn btn-primary leave-save-btn" id="leave-save-btn'+lid+'" data-id="'+lid+'" data-data="'+input_type+'" type="submit">Save</button>');
	...
	$(document).on('click', '.leave-cancel-btn', function() {
		...
		setTimeout(function() {
    		$("#leave_settings"+lid).load(location.href+" #leave_settings"+lid+">*","");
    	}, 1000);
		return false;
	});
	$(document).on('click', '.leave-save-btn', function() {
		...
		if(input_type == '1')
		{
			leave_days = $('#leave_days'+lid).val();
		}

		if(input_type == '2')
		{
			carry_fwd = $('input[name="carry_fwd'+lid+'"]:checked').val();
			if(carry_fwd == '2')
			{
				max_carry_days = $('#max_leaves'+lid).val();
			}
		}

		if(input_type == '3')
		{
			earned_leaves = $('input[name="earned_leaves'+lid+'"]:checked').val();
		}
		...
		$.ajax({
           	...
           	type: "POST",
           	dataType: "json",
           	data: {_token: token, lid:lid, leave_days:leave_days, carry_fwd:carry_fwd, max_carry_days:max_carry_days, earned_leaves:earned_leaves},
           	success:function(response)
            {
            	$("#success_message"+lid).removeClass('d-none');
            	$("#success_message"+lid).show().delay(5000).fadeOut();
	...
	$(document).on('change', '.leave-box .onoffswitch-checkbox', function() {
		...
		$.ajax({
           	...
           	type: "POST",
           	dataType: "json",
           	data: {_token: token, lid:lid, status:val},
           	success:function(response)
            {
            	$("#success_message"+lid).removeClass('d-none');
            	$("#success_message"+lid).show().delay(5000).fadeOut();
